...
//       return res
//         .status(409)
//         .json({ error: `You already submitted an inquiry for this farm and slot(s).` });
//     }
...
//       return res
//         .status(409)
//         .json({ error: `Farm is already confirmed for full day on ${isoDateStr}.` });
//     }
//     if (bookingModes.includes("full_day") && existingModes.length > 0) {
//       return res
//         .status(409)
//         .json({ error: `Farm already has confirmed bookings on ${isoDateStr}.` });
//     }
//     if (
//       (bookingModes.includes("full_night") &&
//         existingModes.includes("night_slot")) ||
...
//       return res
//         .status(409)
//         .json({ error: `Farm already confirmed for a conflicting night slot on ${isoDateStr}.` });
//     }
...
//       const dDate = new Date(d.date);
//       if (isNaN(dDate)) return false;
//       const local = new Date(dDate.getTime() - dDate.getTimezoneOffset() * 60000)
//         .toISOString()
//         .split("T")[0];
//       return local === isoDateStr;
//     });
...
//         return res
//           .status(400)
//           .json({ error: `Pricing not configured for "${mode}" on ${isoDateStr}.` });
//       }
...
//         return res
//           .status(400)
//           .json({ error: `Invalid pricing format for "${mode}" on ${isoDateStr}.` });
//       }
